...
describe("Portfolio API", () => {
  ...
  describe("GET /api/portfolio/:id", () => {
    it("should return a specific portfolio by ID", async () => {
      ...
      const response = await request(app).get(`/api/portfolio/${portfolio._id}`).expect(200)
    ...
    it("should return 404 for non-existent portfolio", async () => {
      ...
      const response = await request(app).get(`/api/portfolio/${nonExistentId}`).expect(404)

      expect(response.body.message).toBe("Portfolio not found")
    })

    it("should return 400 for invalid ObjectId", async () => {
      const response = await request(app).get("/api/portfolio/invalid-id").expect(400)
  ...
  describe("POST /api/portfolio", () => {
    it("should create a new portfolio", async () => {
      ...
      const response = await request(app).post("/api/portfolio").send(portfolioData).expect(201)
    ...
    it("should return 400 when required fields are missing", async () => {
      ...
      const response = await request(app).post("/api/portfolio").send(incompleteData).expect(400)
    ...
    it("should return 400 when portfolio name already exists", async () => {
      ...
      const response = await request(app).post("/api/portfolio").send(duplicateData).expect(400)
  ...
  describe("DELETE /api/portfolio/:id", () => {
    it("should delete an existing portfolio", async () => {
      ...
      const response = await request(app).delete(`/api/portfolio/${portfolio._id}`).expect(200)
    ...
    it("should return 404 for non-existent portfolio", async () => {
      const nonExistentId = new mongoose.Types.ObjectId()
      const response = await request(app).delete(`/api/portfolio/${nonExistentId}`).expect(404)

      expect(response.body.message).toBe("Portfolio not found")
    })

    it("should return 400 for invalid ObjectId", async () => {
      const response = await request(app).delete("/api/portfolio/invalid-id").expect(400)
