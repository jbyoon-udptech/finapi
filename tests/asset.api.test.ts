...
describe("Asset API", () => {
  describe("GET /api/asset", () => {
    it("should return empty array when no assets exist", async () => {
      const response = await request(app).get("/api/asset").expect(200)
  ...
  describe("GET /api/asset/:id", () => {
    ...
    it("should return 404 for non-existent asset", async () => {
      ...
      const response = await request(app).get(`/api/asset/${nonExistentId}`).expect(404)

      expect(response.body.message).toBe("Asset not found")
    })

    it("should return 400 for invalid ObjectId", async () => {
      const response = await request(app).get("/api/asset/invalid-id").expect(400)
  ...
  describe("POST /api/asset", () => {
    it("should create a new asset", async () => {
      ...
      const response = await request(app).post("/api/asset").send(assetData).expect(201)
    ...
    it("should return 400 for invalid category", async () => {
      ...
      const response = await request(app).post("/api/asset").send(invalidData).expect(400)
  ...
  describe("DELETE /api/asset/:id", () => {
    it("should delete an existing asset", async () => {
      ...
      const response = await request(app).delete(`/api/asset/${asset._id}`).expect(200)
    ...
    it("should return 404 for non-existent asset", async () => {
      const nonExistentId = new mongoose.Types.ObjectId()
      const response = await request(app).delete(`/api/asset/${nonExistentId}`).expect(404)

      expect(response.body.message).toBe("Asset not found")
    })

    it("should return 400 for invalid ObjectId", async () => {
      const response = await request(app).delete("/api/asset/invalid-id").expect(400)
